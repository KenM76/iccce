...
- **If another ICC document is needed, ask Ken to download it** — that is the only route. Currently wanted, in priority order: **ICC.1:2010-12** (would settle what changed in `parametricCurveType` Table 68 — corpus rows D10/A31), **ICC.1:2001-04** (the v2 spec — A1b/A2/A34, and the only normative definition of `textDescriptionType`), and ICC's published **D65→D50 `chad`** values cited by Annex E.4.2.
