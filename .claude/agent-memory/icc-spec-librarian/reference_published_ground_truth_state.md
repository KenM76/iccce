...
| ~~sRGB worked triples~~ | **★★★ RETRACTED 2026-08-18 (`C10`) — W3C CSS Color 4 PUBLISHES FOUR**, and the corpus had held that file for one day. `#7654CD`→`lab(44.36% 36.05 -58.99)` **3/3**; `#FFFFFF`→both XYZ whites **6/6**; `srgb`↔`srgb-linear` **3/3**. ⚠ **`xyz-d65` 0/3, `xyz-d50` 1/3, the Lab→sRGB "leaf" 1/3 FAIL their own printed precision** (residuals `1.3e-5`-`1.6e-4`, `ΔE76 0.0123`, QUIET). **Tier `published_literature`, NOT ground truth.** The old row's remaining true half: `srgb.pdf`, W3C 1996, BT.709 and lcms2 publish parameters, never results; the 1996 breakpoint is `0,003 04`/`0,039 28` and all its equations are PNGs. → [[iec-srgb-primary-sourcing-state]] |
...
| **★★★ IEC 61966-2-1 PRIMARY TEXT — pp. 1-15 are FREE** | **HELD 2026-08-18.** IEC's own preview; `webstore.iec.ch/robots.txt` is **HTTP 200, zero bytes**. Gives the **clause map** (5.2 RGB→XYZ, 5.3 XYZ→RGB, **normative**; Annexes A-E **informative**) and **ZERO constants** (0 grep hits for all six). **The gap is now LOCATED, not closed — only CHF 210 answers whether 5.2 prints a matrix or whether `0.04045` is fixed vs derived.** Also revealed **COR1:2014**, previously unknown. → [[iec-srgb-primary-sourcing-state]] |
