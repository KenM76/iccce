...
pub fn u8_fixed8(x: f64) -> [u8; 2] {
    ...
    #[expect(
        ...
        reason = "range checked immediately above: 0.0 <= v <= 65535.0"
    )]
    let bits = v as u16;
    bits.to_be_bytes()
}
...
pub fn legacy_lab_l(l: f64) -> u16 {
    ...
    #[expect(
        clippy::cast_possible_truncation,
        clippy::cast_sign_loss,
        reason = "0..=100 * 652.8 <= 65280 and non-negative, checked above"
    )]
    let bits = v as u16;
    bits
}
...
pub fn legacy_lab_ab(ab: f64) -> u16 {
    ...
    #[expect(
        clippy::cast_possible_truncation,
        clippy::cast_sign_loss,
        reason = "range checked immediately above yields 0..=65535, non-negative"
    )]
    let bits = v as u16;
    bits
}
...
pub fn general_lab_l(l: f64) -> u16 {
    ...
    #[expect(
        clippy::cast_possible_truncation,
        clippy::cast_sign_loss,
        reason = "0..=100 * 655.35 <= 65535 and non-negative, checked above"
    )]
    let bits = v as u16;
    bits
}
